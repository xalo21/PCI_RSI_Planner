...
def _detect_tech(df):
    """Detect technology from 'technology' column if present, else infer from PCI range."""
    if 'technology' in df.columns:
        vals = df['technology'].dropna().astype(str).str.upper().str.strip()
        if vals.str.contains('NR|5G').any():
            return 'NR'
    # Infer from PCI: if any PCI > 503, assume NR
...
def validate_data(df, technology='LTE'):
    """Validate cell data with technology-aware PCI range checking.
    technology: 'LTE' (PCI 0-503) or 'NR' (PCI 0-1007)
    """
    errors = []
    ...
    if 'prach_config_index' in df.columns:
        max_pcfg = 262 if technology == 'NR' else 63
...
def read_excel_file(uploaded_file):
    ...
    try:
        ...
        ok, errs = validate_data(df, technology=_detect_tech(df))
        if not ok:
            for e in errs: msgs.append(f"❌ {e}")
            return None, msgs
        msgs.append("✅ Veri doğrulaması başarılı")
        if 'pci' in df.columns:
...
def generate_sample_excel(technology='LTE'):
    ...
    if is_nr:
        # NR: PCI 0-1007, band n78 (3500 MHz), PRACH config 0-262
    ...
    with pd.ExcelWriter(out, engine='xlsxwriter') as w:
        ...
        if is_nr:
            ...
            prach_desc = 'PRACH Config Index (0-262; 0-27=long, ≥28=short sequence)'
