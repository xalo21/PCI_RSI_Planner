"""
3GPP PCI/RSI Planning Tool v3.0 – Streamlit Web UI
====================================================
...
def _process_cell_excel(file_bytes: bytes, file_name: str, technology: str = 'LTE'):
    """Read + enrich the uploaded cell Excel once per (file content, technology).

    `technology` is the sidebar selection and the sole authority for the run:
    it drives PCI-range validation and the PRACH tables.  It is part of the
    cache key, so switching LTE<->NR re-validates the file."""
    technology = norm_tech(technology)
    df, msgs = read_excel_file(io.BytesIO(file_bytes), technology=technology)
    sg, c2s = None, None
    if df is not None:
        ...
        if (('prach_config_index' in df.columns and 'zero_correlation_zone' in df.columns)
            ...
            df['cell_range_km'] = df.apply(
                lambda r: compute_cell_prach_info(r, technology).get('cell_range_ncs_km', None),
                axis=1)
...
with st.sidebar:
    ...
    tech = norm_tech(technology)
    st.caption(f"🎯 **{tech}** — PCI **0-{pci_max(tech)}** "
               f"(N_ID¹ 0-{sss_count(tech)-1}, N_ID² 0-2) · "
               f"{'TS 38.211 §7.4.2.1' if tech == 'NR' else 'TS 36.211 §6.11'}  \n"
               f"Bu seçim tüm analiz ve planlama için geçerlidir; dosyadaki "
               f"teknoloji etiketi dikkate alınmaz.")
    ...
    if st.button("🗑️ Yeni Oturum — Verileri Temizle", use_container_width=True,
        ...
        st.rerun()
    st.caption("3GPP TS 36.211 / 38.211 • v3.0")
...
_logo_b64 = base64.b64encode(
    (_pathlib.Path(__file__).parent / 'tt_logo.svg').read_bytes()).decode()
st.markdown(
...
with tab1:
    ...
    with c1:
        ...
        if uploaded:
            ...
            df, msgs, sg, c2s = _process_cell_excel(uploaded.getvalue(), uploaded.name, tech)
            for m in msgs:
                (st.error if m.startswith("❌") else st.warning if m.startswith("⚠️")
                 else st.success if m.startswith("✅") else st.info)(m)
            if df is None:
                # Validation failed for the selected technology — drop any
                # previously loaded network so no tab can keep working with
                # data that does not match the current selection.
                for _k in ('df', 'results', 'neighbor_table', 'prach_info',
                           'rsi_plan', 'pci_plan', 'pci_suggestions',
                           'rsi_suggestions', 'rescan_results', 'new_cell_results',
                           'sector_groups', 'cell_to_sector', 'analysis_params',
                           '_plan_cache_key', '_plan_cache_result',
                           '_plan_detail_cache', '_sug_cache_key', '_sug_cache_result'):
                    st.session_state[_k] = None
                st.error(f"❌ Dosya **{tech}** seçimiyle doğrulanamadı "
                         f"(izinli PCI aralığı 0-{pci_max(tech)}). "
                         f"Kenar çubuğundan doğru teknolojiyi seçin veya veriyi düzeltin.")
            else:
    ...
    with c2:
        _tech_lbl = tech
        _pcfg_max = prach_config_max(tech)
        st.markdown(f"""#### 📋 Gerekli Format
Aralıklar seçili teknolojiye (**{_tech_lbl}**) göre gösterilir.
                ...
                with st.expander(f"🔧 Hücre {i+1}", expanded=(i == 0)):
                    ...
                    with mc4:
                        zcz = st.number_input("Zero Correlation Zone", 0, 15, 5,
                                              key=f"nc_zcz_{i}")
                        prach_cfg = st.number_input(
                            "PRACH Config Index", 0, prach_config_max(tech), 0,
                            key=f"nc_prach_{i}",
